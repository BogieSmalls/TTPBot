...
class BotRoomPolicyTests(unittest.TestCase):
    def test_regular_season_form_data_uses_season_goal(self):
        data = race_room_form_data(datetime(2026, 7, 4, 18, 0, tzinfo=TIMEZONE))

        self.assertEqual(data['goal'], GOAL_NAME)
        self.assertIn('TTP Season 4 | Scheduled:', data['info_bot'])

    def test_post_season_form_data_uses_beat_the_game_goal(self):
        data = race_room_form_data(datetime(2026, 7, 6, 20, 0, tzinfo=TIMEZONE))
    ...
    def test_accepts_regular_season_goal_rooms(self):
        self.assertTrue(is_ttp_scheduled_room({
            'goal': {'name': GOAL_NAME},
            'info_bot': 'TTP Season 4 | Scheduled: Sat Jul 04, 06:00 PM EDT',
        }))

    def test_accepts_ttp_labeled_beat_the_game_rooms(self):
        self.assertTrue(is_ttp_scheduled_room({
            'goal': {'name': POST_SEASON_GOAL_NAME},
            'info_bot': 'Triforce Triple Play | Scheduled: Mon Jul 06, 08:00 PM EDT',
        }))
